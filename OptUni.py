...
def checkpoint_file(objective, prop, targets):
    ckpt_dir = os.path.join(PSO_RESULTS_DIR, objective, prop, "checkpoints")
...
class GenerateMolecule:
    ...
    def fitnessFunction(self, latents):
        ...
        properties = self.calProperty(reconstructed)
        fitness = np.abs(properties.T / self.target - 1)
        fitness = fitness.squeeze() * validity
        return fitness.squeeze()

    def run_optimization(self,  target, seed=0, objective="UNI-OBJECTIVE"):
        molProp = self.molProp if type(self.molProp) is str else "-".join(self.molProp)
        self.pso.run(target, seed=seed, propertyName=molProp, multiobjective=False, objective=objective)


# UniObjective
def molopt(seed=0, targets="2.0", prop="logp", objective="UNI-OBJECTIVE", seed_end=100):
    prop_name = prop.upper()
    single_seed_backfill = (seed_end == seed + 1)
    ckpt_path = checkpoint_file(objective, prop_name, targets)
    ...
    final_path = os.path.join(PSO_RESULTS_DIR, objective, prop_name, f"{targets}-Seed{final_seed}.npz")
    if not seed_file_is_healthy(final_path):
        log_info(f"Start target={targets}, prop={prop_name}, objective={objective}, seeds={seed}-{seed_end - 1}")
        while seed < seed_end:
            molGen = None
            seed_ok = False
            path = os.path.join(PSO_RESULTS_DIR, objective, prop_name, f"{targets}-Seed{seed}.npz")
            target = [float(i) for i in targets.split("-")]
            try:
                if not seed_file_is_healthy(path):
                    if os.path.exists(path):
                        os.remove(path)
                    molGen = GenerateMolecule(target=target, molProp=[prop_name], group_size=100)
                    molGen.run_optimization(target, seed=seed, objective=objective)
                    log_info(f"Generated seed={seed} for {objective}/{prop_name}/{targets}")
        ...
        log_info(f"Skip target={targets}, prop={prop_name}, objective={objective} (already complete)")
...
def run_chunk(chunk_idx, chunk_size=100, with_stats=True):
    ...
    objective = f"UNI-OBJECTIVE-R{chunk_idx}"
    ...
    for prop, target in tasks:
        molopt(seed=chunk_seed_start, targets=target, prop=prop, objective=objective, seed_end=chunk_seed_end)

    if with_stats:
        from model_stats import psoValidity, psoGeneratedDistribution, psoDescription
        for prop, target in tasks:
            pso_prop = prop.upper()
            psoValidity(objective=objective, prop=pso_prop, prefix=target,
                        seed_start=chunk_seed_start, n_seeds=chunk_size)
            psoGeneratedDistribution(objective=objective, prop=pso_prop, prefix=target)
            psoDescription(objective=objective, prop=pso_prop, prefix=target)
        log_info(f"Statistics finished for {objective}")


def parse_args():
    parser = argparse.ArgumentParser(description="Run UNI-OBJECTIVE PSO experiments in chunks.")
